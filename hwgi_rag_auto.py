...
INDEX_DIR = os.path.join(SCRIPT_DIR, "Index/faiss_index_bge-m3")  # 인덱스 디렉토리
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "prompt": prompt,
                    "stream": False,
                    "options": {"temperature": 0.1, "top_p": 0.1, "num_predict": 500}  # 온도 낮춤 (0.7 → 0.5)
                }
            )
...
class RAGSystem:
    def __init__(self, embedding_type: str = "bge-m3", use_hnsw: bool = True, ef_search: int = 200, ef_construction: int = 200, m: int = 64):
        ...
        self.index_dir = os.path.join(SCRIPT_DIR, "Index/faiss_index_bge-m3")
...
(위 정보들을 바탕으로 3-4문장으로 답변)"""
    ...
    def _generate_with_ollama(self, prompt: str, model: str, **params) -> str:
        ...
        try:
            ...
            result = response.json().get("response", "")
            elapsed_time = time.time() - start_time
            print(f"✓ 답변 생성 완료: {model} ({elapsed_time:.2f}초)")
            return result
        except Exception as e:
    ...
    def answer(self, query: str, model: str, context: str) -> Dict[str, Any]:
        ...
        params = model_params.get(model, {"temperature": 0.1, "top_p": 0.1, "num_predict": 2048})
        
        # Ollama API를 통해 답변 생성
        print(f"\n🤖 답변 생성 중... ({model})")
        start_time = time.time()
        try:
            response = requests.post(
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "prompt": prompt,
                    "stream": False,
                    "options": {"temperature": 0.2, "top_p": 0.1, "num_predict": 1000}
                }
            )
            response.raise_for_status()
            result = response.json().get("response", "")
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "prompt": prompt,
                    "stream": False,
                    "options": {"temperature": 0.1, "top_p": 0.1, "num_predict": 1000}
                }
            )
            response.raise_for_status()
            result = response.json().get("response", "")
